      ...
      <div class="grid grid-cols-6 gap-4">
        ...
        <div class="h-full mt-[20px] col-start-1 col-end-7 bg-gray-100 ...">
          ...
          <table class="w-full h-full mt-[20px]">
            ...
            <tbody>
            <tr class="m-10 mt-3 border" v-for="index in care_service.data" :key="index">
              <td>{{index.id}}</td>
              <td>
              ...
              <td>
                <router-link :to="{name:'admin-careservice-edit'}"
...
export default {
  setup(){
    ...
    const getCareService = (page=1) => {
      axios.get(`http://127.0.0.1:8000/api/v1/care-service?page=${page}`,{
        headers:{
          Authorization: `Bearer ${localStorage.getItem("jwtToken")}`
        }
      }).then(function (res){
        setAuthHeader(res.apiToken);
        care_service.value = res.data
      })
