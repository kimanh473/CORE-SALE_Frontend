...
} from '@/services/AdminSettingServices/adminsetting.services'
import {
  DataAdminSetting,
  RoleList,
} from '@/store/modules/admin-setting/systemsetting.type'
export const useAdminSetting = defineStore('AdminSetting', {
  ...
  actions: {
    ...
    async getDetailPermissionGroupsAction(
      id: number,
      role: RoleList,
      ...
      await getDetailPermissionGroupsApi(id)
        .then((payload: any) => {
          ...
          role.storeSetting = getMatchingResults(
            'STORE_SETTING',
            res.json_string_roles
          )
          role.product = getMatchingResults(
            'CATALOG_PRODUCT',
            res.json_string_roles
          )
          role.createProduct = getMatchingResults(
            'CATALOG_PRODUCT_CREATE',
            res.json_string_roles
          )
          role.updateProduct = getMatchingResults(
            'CATALOG_PRODUCT_UPDATE',
            res.json_string_roles
          )
          role.deleteProduct = getMatchingResults(
            'CATALOG_PRODUCT_DELETE',
            res.json_string_roles
          )
          role.printProduct = getMatchingResults(
            'CATALOG_PRODUCT_PRINT',
            res.json_string_roles
          )
          role.importProduct = getMatchingResults(
            'CATALOG_PRODUCT_IMPORT',
            res.json_string_roles
          )
          role.exportProduct = getMatchingResults(
            'CATALOG_PRODUCT_EXPORT',
            res.json_string_roles
          )
