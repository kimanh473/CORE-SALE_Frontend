...
import { DataGroupCustomer } from '@/store/modules/customers/customer.type'
export const useGroupCustomer = defineStore('customerGroup', {
  ...
  getters: {
    getListGroupCustomerPagination: (state: any) => {
      return (payload: any) =>
        (state.listGroupCustomer = payload?.map((item: any) => ({
          id: item.id,
          code: item.code,
          title: item.title,
          status: item.status,
    ...
    async createGroupCustomerAction(
      ...
      await createGroupCustomerApi(data)
        ...
        .catch((err) => {
          ...
          console.log(err)
          EndTimeLoading()
          const arrMess = err.response.data.message
