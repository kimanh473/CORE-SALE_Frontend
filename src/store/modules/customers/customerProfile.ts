...
export const useCustomerProfile = defineStore('CustomerProfile', {
  ...
  getters: {
    getListCustomerProfilePagination: (state: any) => {
      return (payload: any) =>
        (state.listCustomerProfile = payload?.map((item: any) => ({
          id: item.id,
          code: item.code,
    ...
    getDetailCustomerProfile: (state: any) => {
      return (payload: any) => {
        ...
        state.idWard = payload.detail_delivery_address?.map(
          (item: any) => item.address_district_id
        )
        state.is_default = payload.detail_delivery_address
          ?.map((item: any) => Number(item.is_default))
          .indexOf(1)
          .toString()
        state.is_default_pay = payload.detail_pay_address
          ?.map((item: any) => Number(item.is_default_pay))
          .indexOf(1)
          .toString()
  ...
  actions: {
    ...
    getLastCodeCustomer() {
      getLastCodeCustomerApi().then((payload: any) => {
        const res = payload?.data
        this.getCodeCustomer(res)
      })
    },
    getCheckAccountCustomerExist(code: any) {
      getCheckAccountCustomerExistApi(code).then((payload: any) => {
        const res = payload?.data
